...
export default function AesCrypto() {
  ...
  // 生成密钥
  const generateKey = () => {
    generateAesKey(store.bitSize, store.key.encoding)
      .catch(stringify)
  ...
  // 生成向量
  const generateIv = () => {
    generateAesIv(store.bitSize, store.blockMode, store.iv.encoding)
      .catch(stringify)
